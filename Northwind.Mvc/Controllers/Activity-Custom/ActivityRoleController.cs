                        ...
                        if (Application.Create(activityRoleItemModel.OperationResult, activityRoleDTO))
                        {
                            if (activityRoleItemModel.IsSave)
                            {
                                ...
                                return JsonResultSuccess(activityRoleItemModel.OperationResult,
                                    Url.Content(String.Format("~/ActivityRole/Update?ActivityId={0}&RoleName={1}", activityRoleDTO.ActivityId, activityRoleDTO.RoleName)));
                            }
                            else
                            {
                                return JsonResultSuccess(activityRoleItemModel.OperationResult);
        ...
        public ActionResult DataSource(DataManager dataManager)
        {
            ...
            if (IsSearch())
            {
                try
                {
                    ...
                    data = ZViewHelper<ActivityRoleViewModel, ActivityRoleDTO, ActivityRole>.ToViewList(Application.Select(operationResult, where, args.ToArray(), orderBy, dataManager.Skip, take));
